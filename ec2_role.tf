...
resource "aws_iam_instance_profile" "k3s_server" {
  name = "K8S-ServerProfile"
  role = aws_iam_role.k3s_server.name
  tags = local.terratag_added_ec2_role
}
...
locals {
  terratag_added_ec2_role = {"Project"="K8S-Demo","kubernetes.io/cluster/kubernetes"="owned"}
}
